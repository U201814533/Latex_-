\documentclass{mcmthesis}	%mcm格式
\mcmsetup{CTeX =false,   % 使用 CTeX 套装时，设置为 true
        tcn = 000000, problem = A,%队伍控制号和队伍的题目选择
        sheet = true, titleinsheet =true, keywordsinsheet = true,
        titlepage =false, abstract = false}%表格摘要等信息选择
\usepackage{palatino}
\usepackage{lipsum}
...
\geometry{left=3.0cm,right=3.0cm}%利用geometry宏包控制页边距

\begin{document}%开始显示文档的内容
...
\setcounter{page}{1}
\section{Introduction}%Introduction节
\subsection{Background}%Background子节
...
\subsection{Problem Restatement}%Problem Restatement子节
...
\subsection{Our Work}%Our Work子节
...
\section{Assumptions}%Assumptions节

\begin{itemize}%设置子项目，还可以设置item前面显示格式
...
\section{List of Notation}%符号标志列表

\begin{center}%表格名称居中
\begin{longtable}{p{.1\textwidth}p{.8\textwidth}m{.4\textwidth}}
\caption{The List of Notation}\\%给出表格的名称，其中表格的名称应该在表的上方
\hline%画出水平线
Symbol& Meaning \\%分别给出符号和意义
\hline
...
\begin{table}[!ht]%设置表格放置的位置
\caption{The categories of hurricanes}%输出表格的名称
 \renewcommand\arraystretch{1.5}
 \setlength{\abovecaptionskip}{0pt}%    
\setlength{\belowcaptionskip}{10pt}%
%设置对应的字体样式和位置关系
\begin{center}%设置居中
\begin{tabular}{p{.2\textwidth}p{.2\textwidth}m{.5\textwidth}}%分别设置三列的列宽
\toprule[1.5pt]%设置线条的宽度
Category& Maximum sustained winds & Potential damage \\
 \midrule%线条的宽度使用，默认值为0.5pt
...
%
  Category 5 & $ \geq $ 250 km/h &   Most of the buildings and detached houses were completely destroyed, and some houses were blown away completely.Flood have devastated large areas, and all buildings near the coast have been flooded and settlers may need to evacuate. \\  \bottomrule[1.5pt]%设置线条的宽度
 \end{tabular}
 \end{center} 
 \end{table}
...
\begin{figure}[tbp]%设置图片放置的位置
  \centering{%居中放置
  \includegraphics[width=0.6\textwidth]{./picture/figure1.eps}}%插入图片的命令
  \caption{The Extent of the Hurricane in Mississippi}\label{figure1}
  %设置图的名称并且设置一个图的标签，方便后面引用
\end{figure}
...
C(t) = \frac{1}{{(1 + {e^{ - \alpha (t - h)}})}}%设置一个等式
\end{equation}
...
In the case of low loading rate, people gradually reach the station node within the time range of evacuation, but for the high loading rate, a large number of people rush to the station node during the final phase of evacuation,as is shown in Figure ~\ref{fig:2}.%设置一个引用

\begin{figure}[htbp]%设置图标的放置位置
  \centering%居中设置
  \includegraphics[width=0.7\textwidth]{./picture/figure2.eps}
  \caption{Behavioral Response Curves}\label{fig:2}%设置引用
 \end{figure}
...
\addcontentsline{toc}{section}{Reference}%设置添加参考文献
\bibliographystyle{plain}%利用BibTeX插入对应的从参考文献
